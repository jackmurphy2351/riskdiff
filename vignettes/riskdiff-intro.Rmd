...
The second statement is immediately actionable: in a screening camp of 1,000 people, you would expect to find approximately 120 additional cancer cases among betel nut chewers compared to non-chewers. This directly informs:
...
cachar_sample %>%
  select(abnormal_screen, chewer, smoker, drinker, age, sex, residence) %>%
  summary()
```
...
-   `chewer` indicates tobacco chewing status
...
exposure_summary <- cachar_sample %>%
  ...
  summarise(
    n = n(),
    cases = sum(diagnosis_bin),
    risk = mean(diagnosis_bin),
...
comparison <- bind_rows(
  rd_simple %>% mutate(model = "Unadjusted"),
  rd_adjusted %>% mutate(model = "Adjusted")
) %>%
  select(model, rd, ci_lower, ci_upper, p_value)
...
rd_identity <- calc_risk_diff(
  cachar_sample, "diagnosis_bin", "chewer", 
  link = "identity"
)
...
rd_missing <- calc_risk_diff(
  ...
  "diagnosis_bin",
  "chewer",
  adjust_vars = c("age", "white_discharge")  # white_discharge has missing values
)

cat("Complete cases used:", rd_missing$n_obs, "out of", nrow(cachar_sample), "\n")
```
...
calc_risk_diff(
  ...
  exposure,                # Exposure of interest
  adjust_vars = NULL,      # Variables to adjust for
  strata = NULL,          # Stratification variables
  link = "auto",          # Link function: "auto", "identity", "log", "logit"
  alpha = 0.05,           # Significance level (0.05 = 95% CI)
  verbose = FALSE         # Print diagnostic messages
)
```
...
-   **Issues or bugs**: [https://github.com/[your-username]/riskdiff/issues](https://github.com/%5Byour-username%5D/riskdiff/issues)
